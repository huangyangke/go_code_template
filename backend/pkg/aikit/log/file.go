package log

import (
	"context"
	"io"
	"path/filepath"
	"time"

	"github.com/huangyangke/go-aikit/log/internal/filewriter"
)

const (
	_debugIdx = iota
	_infoIdx
	_warnIdx
	_errorIdx
	_fatalIdx
	_totalIdx
)

var _fileNames = map[int]string{
	_debugIdx: "debug.log",
	_infoIdx:  "info.log",
	_warnIdx:  "warning.log",
	_errorIdx: "error.log",
	_fatalIdx: "fatal.log",
}

// FileHandler 文件日志处理器，按级别分文件写入.
type FileHandler struct {
	render Render
	fws    [_totalIdx]*filewriter.FileWriter
}

// NewFile 创建文件日志处理器.
// 参数：dir - 日志目录, rotateSize - 单文件最大字节数, maxLogFile - 最大保留文件数.
// 返回值：文件日志处理器.
func NewFile(dir string, rotateSize int64, maxLogFile int) *FileHandler {
	newWriter := func(name string) *filewriter.FileWriter {
		var options []filewriter.Option
		if rotateSize > 0 {
			options = append(options, filewriter.MaxSize(rotateSize))
		}
		if maxLogFile > 0 {
			options = append(options, filewriter.MaxFile(maxLogFile))
		}
		w, err := filewriter.New(filepath.Join(dir, name), options...)
		if err != nil {
			panic(err)
		}
		return w
	}
	handler := &FileHandler{
		render: newPatternRender("[%T] [%L] [%S] %M"),
	}
	for idx, name := range _fileNames {
		handler.fws[idx] = newWriter(name)
	}
	return handler
}

// Log 将日志写入对应级别的文件.
// 参数：ctx - 上下文, lv - 日志级别, args - 结构化字段列表.
func (h *FileHandler) Log(ctx context.Context, lv Level, args ...D) {
	d := toMap(args...)
	d[_time] = time.Now().Format(timeFormat)
	var w io.Writer
	switch lv {
	case _debugLevel:
		w = h.fws[_debugIdx]
	case _warnLevel:
		w = h.fws[_warnIdx]
	case _errorLevel:
		w = h.fws[_errorIdx]
	case _fatalLevel:
		w = h.fws[_fatalIdx]
	default:
		w = h.fws[_infoIdx]
	}
	h.render.Render(w, d) //nolint:errcheck
	w.Write([]byte("\n")) //nolint:errcheck
}

// Close 关闭所有文件写入器.
// 返回值：err - 关闭失败时的错误.
func (h *FileHandler) Close() error {
	for _, fw := range h.fws {
		func() { _ = fw.Close() }()
	}
	return nil
}

// SetFormat 设置日志输出格式模板.
// 参数：format - 格式模板字符串.
func (h *FileHandler) SetFormat(format string) {
	h.render = newPatternRender(format)
}
