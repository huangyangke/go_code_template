// Package filewriter 支持日志轮转的异步文件写入器.
package filewriter
...
func (w *wrapFile) size() int64 { return w.fsize }
...
// New 创建 FileWriter.
// 参数：fpath - 文件路径, fns - 配置选项列表.
// 返回值：FileWriter 实例, err - 创建失败时的错误.
