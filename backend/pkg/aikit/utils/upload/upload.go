...
// New 创建 Uploader，panic on nil config or empty required fields.
...
// Upload 上传文件，返回 URL 和文件 ID。
// 优先用 filePath，其次用 fileBytes + fileName。
...
// multipart form helper (unused but available for callers needing form upload)
func buildForm(fields map[string]string, fileField, fileName string, fileData []byte) (io.Reader, string, error) {
