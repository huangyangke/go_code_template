...
func TestMarkQueuedForRetry(t *testing.T) {
	mr, store := setupStatusStore(t)
	rdb := redis.NewClient(&redis.Options{Addr: mr.Addr()})
	t.Cleanup(func() { rdb.Close() })
	ctx := context.Background()
	...
	// Seed a running task first so there is an existing hash to update
	require.NoError(t, store.InitQueued(ctx, "task-retry", "/ep", 5))
	require.NoError(t, store.MarkRunning(ctx, "task-retry"))

	// Drain the running event before we test MarkQueuedForRetry
	_ = receiveEvent(t, ch)

	require.NoError(t, store.MarkQueuedForRetry(ctx, "task-retry", "retrying soon", "transient error"))

	// Verify persisted fields
	ts, err := store.Get(ctx, "task-retry")
	require.NoError(t, err)
	require.NotNil(t, ts)
	assert.Equal(t, TaskStatusQueued, ts.Status)
	assert.Equal(t, "retrying soon", ts.Message)
	assert.Equal(t, "transient error", ts.Error)

	// Verify event was published
	ev := receiveEvent(t, ch)
	assert.Equal(t, TaskStatusQueued, ev["status"])
	assert.Equal(t, "task-retry", ev["task_id"])
}

func TestPublishTaskEvent_MarshalError_DoesNotPanic(t *testing.T) {
	_, store := setupStatusStore(t)
	ctx := context.Background()

	// Build a TaskStatus whose Result field is an invalid JSON fragment.
	// decodeResult will wrap it as json.RawMessage; json.Marshal will fail,
	// but PublishTaskEvent (and publishTaskEvent) must not panic.
	status := &TaskStatus{
	...
	assert.NotPanics(t, func() {
		// PublishTaskEvent returns an error on marshal failure but must not panic.
		_ = store.PublishTaskEvent(ctx, "task-bad", status)
	})
}

// ── Redis error paths ─────────────────────────────────────────────────────────

func TestInitQueued_RedisError(t *testing.T) {
	mr, store := setupStatusStore(t)
	mr.Close() // force error
	err := store.InitQueued(context.Background(), "t", "/ep", 5)
	assert.Error(t, err)
}

func TestMarkRunning_RedisError(t *testing.T) {
	mr, store := setupStatusStore(t)
	require.NoError(t, store.InitQueued(context.Background(), "t", "/ep", 5))
	mr.Close()
	err := store.MarkRunning(context.Background(), "t")
	assert.Error(t, err)
}

func TestMarkFailed_RedisError(t *testing.T) {
	mr, store := setupStatusStore(t)
	require.NoError(t, store.InitQueued(context.Background(), "t", "/ep", 5))
	mr.Close()
	err := store.MarkFailed(context.Background(), "t", "oops")
	assert.Error(t, err)
}

func TestMarkCancelled_RedisError(t *testing.T) {
	mr, store := setupStatusStore(t)
	require.NoError(t, store.InitQueued(context.Background(), "t", "/ep", 5))
	mr.Close()
	err := store.MarkCancelled(context.Background(), "t", "reason")
	assert.Error(t, err)
}

func TestMarkQueuedForRetry_RedisError(t *testing.T) {
	mr, store := setupStatusStore(t)
	require.NoError(t, store.InitQueued(context.Background(), "t", "/ep", 5))
	mr.Close()
	err := store.MarkQueuedForRetry(context.Background(), "t", "retry", "timeout")
	assert.Error(t, err)
}

func TestUpdateProgress_RedisError(t *testing.T) {
	mr, store := setupStatusStore(t)
	require.NoError(t, store.InitQueued(context.Background(), "t", "/ep", 5))
	mr.Close()
	err := store.UpdateProgress(context.Background(), "t", 50, "halfway")
	assert.Error(t, err)
}
...
func TestGet_RedisError(t *testing.T) {
	mr, store := setupStatusStore(t)
	mr.Close()
	_, err := store.Get(context.Background(), "t")
	assert.Error(t, err)
}

func TestPublishTaskEvent_GetError(t *testing.T) {
	mr, store := setupStatusStore(t)
	ctx := context.Background()
	require.NoError(t, store.InitQueued(ctx, "t", "/ep", 5))
	ts, _ := store.Get(ctx, "t")
	mr.Close()
	// PublishTaskEvent should return error when Redis is down
	err := store.PublishTaskEvent(ctx, "t", ts)
	assert.Error(t, err)
}

func TestCancelIfQueued_RedisError(t *testing.T) {
	mr, store := setupStatusStore(t)
	mr.Close()
	_, err := store.CancelIfQueued(context.Background(), "t", "reason")
	assert.Error(t, err)
}
