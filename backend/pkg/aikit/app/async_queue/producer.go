...
// TaskEventDispatcher manages SSE connections and event routing
...
func (d *TaskEventDispatcher) run() {
	defer func() {
		// Allow restart on next subscribe() call after disconnect.
		d.mu.Lock()
		d.running = false
		d.mu.Unlock()
	}()
...
// RegisterRoutes 在 Gin router 上注册所有 endpoint 路由
// 同时注册 /status/:task_id 和 /cancel/:task_id
...
func (p *Producer) handleEvents(c *gin.Context) {
	...
	for {
		...
		case <-c.Request.Context().Done():
			log.Info("[Producer][events][disconnect][task_id=%s]", taskID)
			return
