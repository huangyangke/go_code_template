...
// Consumer 从 Redis Stream 读取任务并执行
//
// 两层并发控制：
//
//	L1: 全局 semaphore（worker_capacity），所有 endpoint 共享
//	L2: 每个 endpoint 的 ConcurrencyLimiter（local 或 distributed）
...
func (c *Consumer) Start(ctx context.Context) error {
	...
	if err := c.createGroup(ctx); err != nil {
		log.Error("[Consumer][create_group_error][consumer=%s][group=%s][stream=%s]: %v",
			consumerName, c.groupName, c.cfg.StreamKey, err)
		return err
	}
...
func (c *Consumer) loop(ctx context.Context, consumerName string) error {
	...
	for {
		...
		msgs, err := c.rdb.XReadGroup(ctx, &redis.XReadGroupArgs{
			...
			Streams:  []string{c.cfg.StreamKey, StreamNewMessagesID},
			Count:    pullCount,
			Block:    DefaultPullBlock,
		}).Result()
		if err != nil && err != redis.Nil {
			log.Warn("[Consumer][xreadgroup_error][consumer=%s][stream=%s]: %v", consumerName, c.cfg.StreamKey, err)
...
func (c *Consumer) ackAndDel(ctx context.Context, msgID string) error {
	pipe := c.rdb.Pipeline()
	pipe.XAck(ctx, c.cfg.StreamKey, c.groupName, msgID)
	pipe.XDel(ctx, c.cfg.StreamKey, msgID)
	_, err := pipe.Exec(ctx)
	return err
}
