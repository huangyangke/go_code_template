...
import (
	...
	"github.com/example/go-template/pkg/aikit/app/async_queue"
...
// FastAppConfig holds FastApp configuration
...
//   - Gin HTTP server with built-in /healthz and /metrics endpoints
//   - Middleware suite (RequestID, RequestLog, Prometheus, CORS, TokenAuth, RateLimit)
//   - Async queue (Producer + Consumer) lifecycle management
//   - Resource accessors (Redis, MySQL, Cache)
...
// NewFastApp creates a new FastApp instance
...
// Engine returns the underlying Gin engine for custom route registration
func (a *FastApp) Engine() *gin.Engine {
	return a.engine
}

// SetRouteRegistrar sets a function that registers custom routes on the engine.
...
// SetAuth registers a pre-built auth.Manager and its routes.
// Call before Run().
func (a *FastApp) SetAuth(m *auth.Manager) {
	a.authManager = m
}

// AuthManager returns the registered auth.Manager, or nil.
func (a *FastApp) AuthManager() *auth.Manager {
	return a.authManager
}

// SetAsyncQueue configures async queue integration
...
// SetXxlJob configures XXL-Job integration
func (a *FastApp) SetXxlJob(cfg XxlJobConfig) {
	a.xjobConfig = &cfg
}

// RegisterXxlJobTask registers a task with the XXL-Job executor
...
// GetXxlJob returns the XXL-Job executor
...
// GetRedis returns a named Redis instance
...
// GetMySQL returns a named MySQL instance
...
// RegisterHTTPClient registers a named HTTP client instance
...
// Family returns the service family name
...
func (a *FastApp) setupAsyncQueue() error {
	...
	// Auto-fill stream key from family
	if cfg.RedisConfig.StreamKey == "" && a.cfg.Family != "" {
		cfg.RedisConfig.StreamKey = fmt.Sprintf("aikit:async:%s:stream", a.cfg.Family)
	}
